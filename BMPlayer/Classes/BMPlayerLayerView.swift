...
open class BMPlayerLayerView: UIView {
    ...
    // MARK: - 计时器事件
    var globalTotalTime : TimeInterval = 0
    @objc fileprivate func playerTimerAction() {
        if let playerItem = playerItem {
            //cambio 3
            //if playerItem.duration.timescale != 0 {
            if playerItem.currentTime().timescale != 0 {
                let currentTime = CMTimeGetSeconds(self.player!.currentTime())
                //let totalTime   = TimeInterval(playerItem.duration.value) / TimeInterval(playerItem.duration.timescale)
               // let totalTime   = TimeInterval(playerItem.currentTime().value) / TimeInterval(playerItem.currentTime().timescale)
                if(currentTime>globalTotalTime){
                    globalTotalTime = currentTime
                }
                delegate?.bmPlayer(player: self, playTimeDidChange: currentTime, totalTime: globalTotalTime)
            }
            ...
            if item == self.playerItem {
                ...
                case "loadedTimeRanges":
                    ...
                    if let timeInterVarl    = self.availableDuration() {
                        ...
                        delegate?.bmPlayer(player: self, loadedTimeDidChange: timeInterVarl, totalDuration: self.globalTotalTime)
                    }
